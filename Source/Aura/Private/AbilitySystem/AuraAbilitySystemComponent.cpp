...
void UAuraAbilitySystemComponent::AddCharacterAbilities(const TArray<TSubclassOf<UGameplayAbility>>& StartupAbilities)
{
	for (const TSubclassOf<UGameplayAbility> AbilityClass : StartupAbilities)
	{
		...
		if (const UAuraGameplayAbility* AuraAbility = Cast<UAuraGameplayAbility>(AbilitySpec.Ability))
		{
			...
			AbilitySpec.GetDynamicSpecSourceTags().AddTag(FAuraGameplayTags::Get().Abilities_Status_Assigned);
...
void UAuraAbilitySystemComponent::ServerSpendSpellPoint_Implementation(const FGameplayTag& AbilityTag)
{
	if (FGameplayAbilitySpec* AbilitySpec = GetAbilitySpecFromTag(AbilityTag))
	{
		...
		else if (Status.MatchesTagExact(GameplayTags.Abilities_Status_Assigned) ||
...
void UAuraAbilitySystemComponent::ServerAssignAbility_Implementation(const FGameplayTag& AbilityTag, const FGameplayTag& Slot)
{
	if (FGameplayAbilitySpec* AbilitySpec = GetAbilitySpecFromTag(AbilityTag))
	{
		const FAuraGameplayTags& GameplayTags = FAuraGameplayTags::Get();
		...
		const bool bStatusValid = Status == GameplayTags.Abilities_Status_Assigned ||
			Status == GameplayTags.Abilities_Status_Unlocked;
		if (bStatusValid)
		{
			// Handle Activation and Deactivation for passive abilities

			if (!SlotIsEmpty(Slot)) // There is already an ability in this slot -> Deactivate and clear
			{
				if (FGameplayAbilitySpec* SpecWithSlot = GetAbilitySpecWithSlot(Slot))
				{
					// If abilities are the same, return early
					if (AbilityTag.MatchesTagExact(GetAbilityTagFromSpec(*SpecWithSlot)))
					{
						ClientAssignAbility(AbilityTag, GameplayTags.Abilities_Status_Assigned, Slot, PrevSlot);
						return;
					}
			...
			if (!AbilityHasAnySlot(*AbilitySpec)) // Ability doesn't have any slot yet (not active)
			{
